...
const server = Bun.serve({
	...
	fetch(req) {
		...
		if (staticFiles.some(file => url.pathname.startsWith(file.split('/')[1]))) {
			...
			setTimeout(() => {
				console.log(`[${process.env.SERVICE_NAME}] Static file: ${url.pathname} (${responseTime.toFixed(0)}ms)`);
			}, 0);
		...
		if (pages.includes(url.pathname) || url.pathname === '/') {
			...
			setTimeout(() => {
				console.log(`[${process.env.SERVICE_NAME}] Page: ${url.pathname} - ${isMobile ? 'Mobile' : 'Desktop'} (${responseTime.toFixed(0)}ms)`);
			}, 0);
...
setInterval(() => {
	if (Math.random() > 0.6) {
		...
		console.log(`[${process.env.SERVICE_NAME}] Stats: ${uniqueVisitors} unique visitors, ${staticFileRequests} static files served`);
	}

	if (Math.random() > 0.8) {
		console.log(`[${process.env.SERVICE_NAME}] Server load: ${(Math.random() * 100).toFixed(1)}% CPU, ${(Math.random() * 80 + 20).toFixed(1)}% Memory`);
	}
