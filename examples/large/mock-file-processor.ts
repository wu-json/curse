...
const statuses = ["processing", "completed", "failed", "queued"];
...
function simulateFileProcessing() {
	if (Math.random() > 0.6) {
		...
		const directory =
			watchedDirectories[Math.floor(Math.random() * watchedDirectories.length)];
		const fileSize = (Math.random() * 50 + 1).toFixed(2);

		console.log(
			`[FILE-PROCESSOR] New file detected: ${directory}/${fileName} (${fileSize}MB)`,
		);
		console.log(
			`[FILE-PROCESSOR] Starting ${fileType.action} for ${fileName}...`,
		);
		...
		setTimeout(() => {
			if (Math.random() > 0.15) {
				processedFiles++;
				console.log(
					`[FILE-PROCESSOR] ${fileType.action} completed for ${fileName}`,
				);
	...
	if (Math.random() > 0.85) {
		...
		console.log(
			`[FILE-PROCESSOR] Scanning ${directory}... found ${fileCount} new files`,
		);
	}
...
console.log(
	"[FILE-PROCESSOR] Watching directories:",
	watchedDirectories.join(", "),
);
console.log(
	"[FILE-PROCESSOR] Supported formats: PDF, JPG, MP4, CSV, XLSX, ZIP, DOCX",
);
...
process.on("SIGTERM", () => {
	console.log(
		"[FILE-PROCESSOR] Received SIGTERM, finishing current processing...",
	);
...
process.on("SIGINT", () => {
	console.log(
		"[FILE-PROCESSOR] Received SIGINT, finishing current processing...",
	);
	console.log("[FILE-PROCESSOR] File processor stopped");
	process.exit(0);
});
