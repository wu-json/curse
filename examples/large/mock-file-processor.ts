...
function simulateFileProcessing() {
  if (Math.random() > 0.6) {
    ...
    setTimeout(() => {
      if (Math.random() > 0.15) {
        ...
        console.log(`[FILE-PROCESSOR] ${fileType.action} failed for ${fileName}: Corrupted file format`);
      }
    }, processingTime);
  }
  ...
  if (Math.random() > 0.9) {
    console.log(`[FILE-PROCESSOR] Status: ${processedFiles} files processed, disk usage: ${(Math.random() * 40 + 30).toFixed(1)}%`);
  }
}
